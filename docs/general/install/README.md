...
```hcl
module "ecr" {
    source = "github.com/pbs/terraform-ecr-module?ref=x.y.z"
}
```
...
gh -R 'pbs/terraform-ecr-module' release download -p 'release.tar.gz' x.y.z
tar -xvf release.tar.gz -C 'terraform/modules/ecr'
rm -f release.tar.gz
```
...
```bash
git remote add -f ecr git@github.com:pbs/terraform-ecr-module.git
git subtree add --prefix terraform/modules/ecr ecr main --squash
```

If necessary, pin the repo version, rather than adding `main` as your subtree:

```bash
git subtree add --prefix terraform/modules/ecr ecr $REF --squash
```
